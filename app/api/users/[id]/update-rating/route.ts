...
// POST - Recalculate and update usher's overall rating and stats
export async function POST(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    const usherId = parseInt(params.id)
    
    if (isNaN(usherId)) {
      return NextResponse.json({ error: 'Invalid usher ID' }, { status: 400 })
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Usher rating updated successfully',
      stats: {
        ...
        totalGigsCompleted: parseInt(attendanceStats[0]?.total_shifts_completed) || 0,
