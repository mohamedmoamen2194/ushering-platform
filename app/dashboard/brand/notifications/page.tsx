...
import { Bell, RefreshCw, CheckCheck, Briefcase, CreditCard, Star, MessageCircle, Users } from "lucide-react"
...
export default function BrandNotificationsPage() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "gig_alert": return <Briefcase className="h-4 w-4 text-secondary" />
      case "application": return <Users className="h-4 w-4 text-accent" />
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
        <div>
          ...
          <p className="text-xs text-muted-foreground font-light mt-0.5">{language === "ar" ? `Ã™â€žÃ˜Â¯Ã™Å Ã™Æ’ ${unreadCount} Ã˜Â¬Ã˜Â¯Ã™Å Ã˜Â¯` : `${unreadCount} new`}</p>
        </div>
        <div className="flex gap-1">
          {unreadCount > 0 && <Button variant="ghost" size="sm" onClick={markAllRead}><CheckCheck className="h-4 w-4 mr-1" /><span className="text-xs hidden sm:inline">{language === "ar" ? "Ã™â€šÃ˜Â±Ã˜Â§Ã˜Â¡Ã˜Â© Ã˜Â§Ã™â€žÃ™Æ’Ã™â€ž" : "Read All"}</span></Button>}
          <Button variant="ghost" size="sm" onClick={fetchNotifications}><RefreshCw className="h-4 w-4" /></Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="animate-fade-in-up border-dashed" style={{ animationDelay: "0.1s" }}>
          <CardContent className="flex flex-col items-center py-14"><Bell className="h-7 w-7 text-muted-foreground/40 mb-4" /><p className="text-sm font-semibold text-muted-foreground">{language === "ar" ? "Ã™â€žÃ˜Â§ Ã˜ÂªÃ™Ë†Ã˜Â¬Ã˜Â¯ Ã˜Â¥Ã˜Â´Ã˜Â¹Ã˜Â§Ã˜Â±Ã˜Â§Ã˜Âª" : "No notifications"}</p></CardContent>
        </Card>
      ) : (
        <div className="space-y-1.5">
          {notifs.map((n, i) => (
            <Card key={n.id} className={`animate-fade-in-up border-l-4 ${n.is_read ? "border-l-transparent opacity-60" : "border-l-secondary"}`} style={{ animationDelay: `${i * 0.03}s` }}>
              <CardContent className="p-3 sm:p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2"><p className="text-sm font-semibold">{n.title}</p><span className="text-[10px] font-mono text-muted-foreground/50 shrink-0">{formatTime(n.created_at)}</span></div>
