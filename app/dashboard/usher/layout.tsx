...
export default function UsherDashboardLayout({
  children,
}: {
      ...
      <div className={`min-h-screen bg-background ${isRTL ? "font-arabic" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
        <header className="bg-card/70 backdrop-blur-md border-b border-border/50 sticky top-0 z-40">
          <div className="container mx-auto px-4 max-w-7xl flex items-center justify-end gap-1.5 py-2.5">
            <span className="mr-auto text-xs font-mono font-semibold text-muted-foreground/50 tracking-widest uppercase hidden sm:block">
              {language === "ar" ? "لوحة التحكم" : "Dashboard"}
            </span>
            <ThemeToggle />
            <NotificationDropdown role="usher" />
            <LanguageSwitcher />
        ...
        <BottomNav role="usher" language={language} />
      </div>
    </ProtectedRoute>
  )
}
